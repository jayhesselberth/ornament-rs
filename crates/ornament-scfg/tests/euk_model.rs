...
fn setup() -> (ornament_scfg::Cm, Vec<ornament_alphabet::Dsq>, usize) {
...
fn euk_model_cyk_and_inside_match_oracle() {
    ...
    let cyk = cyk_scan_glocal(&cm, &dsq, cm.w as usize)
        .best
        .expect("cyk hit");
    eprintln!("euk CYK: score={:.3} {}..{}", cyk.score, cyk.i, cyk.j);
    assert_eq!((cyk.i, cyk.j), (1, 90));
    assert!(
        (cyk.score - 114.0).abs() < 0.2,
        "CYK {:.3} != 114.0",
        cyk.score
    );

    let ins = inside_scan_glocal(&cm, &dsq, cm.w as usize)
        .best
        .expect("inside hit");
    eprintln!("euk Inside: score={:.3}", ins.score);
    assert!(
        (ins.score - 114.2).abs() < 0.25,
        "Inside {:.3} != 114.2",
        ins.score
    );
...
fn euk_model_alignment_matches_oracle_structure() {
    ...
    assert!(
        cols.iter().all(|&c| (1..=90).contains(&c)),
        "columns in range"
    );
