...
use ornament_scfg::{align_glocal, configure_scores, parse_cm_file, EmitMap};
...
fn emit_map_spans_consensus() {
    ...
    for nd in 0..cm.nodes {
        if matches!(
            cm.ndtype[nd],
            ornament_scfg::model::nd::MATP | ornament_scfg::model::nd::MATL
        ) {
            assert!((1..=71).contains(&emap.lpos[nd]), "lpos in range");
        }
        if matches!(
            cm.ndtype[nd],
            ornament_scfg::model::nd::MATP | ornament_scfg::model::nd::MATR
        ) {
