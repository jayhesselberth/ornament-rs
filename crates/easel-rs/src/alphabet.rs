...
impl Alphabet {
    ...
    pub fn rna_with_modifications(mods: &[ModSpec]) -> Result<Self, EaselError> {
    ...
    fn nucleic_checked(atype: AlphabetType, mods: &[ModSpec]) -> Result<Self, EaselError> {
        ...
        let (canon, complement_canon): (&[u8], &[u8]) = match atype {
            ...
            AlphabetType::Custom => {
                return Err(EaselError::Alphabet("custom nucleic not supported".into()))
            }
        };
        let k = canon.len(); // 4
    ...
    pub fn digitize(&self, text: &str) -> Result<Vec<Dsq>, EaselError> {
    ...
    pub fn revcomp(&self, dsq: &mut [Dsq]) -> Result<(), EaselError> {
        let comp = self
            .complement
            .as_ref()
            .ok_or_else(|| EaselError::Alphabet("alphabet has no complement".into()))?;
